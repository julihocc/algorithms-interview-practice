 ...
 * ];
 *
 * ); # [ { x: 3, y: 2 }, { x: 3, y: 5 }, { x: 4, y: 5 } ]
 */

import {expect} from "chai";
import {findIntersections, HorizontalSegment, VerticalSegment} from './solution';
...
describe('findIntersections', () => {
    test('finds intersections', () => {
        ...
        expect(findIntersections(horizontalSegments, verticalSegments)).to.deep.equal([{x: 3, y: 2}, {
            x: 3,
            y: 5
        }, {x: 4, y: 5}]);
    });
